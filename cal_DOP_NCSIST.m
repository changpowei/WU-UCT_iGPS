function [percent] = cal_DOP(BS_lats, BS_lons, RX_lat_bound1, RX_lat_bound2, RX_lon_bound1, RX_lon_bound2);

c = physconst('lightspeed');

reference_lat = (RX_lat_bound1+RX_lat_bound2)/2;
reference_lon = (RX_lon_bound1+RX_lon_bound2)/2;

tx_latitudes = [BS_lats, reference_lat];
tx_longitudes = [BS_lons, reference_lon];

% 以1km為單位設置Rx
rx_latitudes = (RX_lat_bound1:0.01:RX_lat_bound2)';
rx_longitudes = (RX_lon_bound1:0.01:RX_lon_bound2)';

%
tx = txsite('Name','iGPS Tx',...
    'Antenna',dipole, ...
    'Latitude',tx_latitudes(:), ...
    'Longitude',tx_longitudes(:), ...
    'AntennaHeight',3, ...
    'TransmitterFrequency',3775e6, ...
    'TransmitterPower',50);
tx_elevations = elevation(tx);

% Tx轉直角座標
for itx = 1:size(tx_latitudes,2),
    tx_xyz(:,itx) = llh2xyz([tx_latitudes(itx) tx_longitudes(itx) tx_elevations(itx)]);
end;
% 選擇第五個Tx作為參考點
orgxyz = tx_xyz(:,5);
% Tx轉ENU座標
